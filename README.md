...
|------|------|---------|--------------|
| **PyTorch** | 灵活、易调试 | 学术研究、快速原型 | ⭐ 85k+ |
| **TensorFlow** | 生产部署强 | 工业级部署 | ⭐ 186k+ |
| **OpenCV** | 图像处理基础库 | 预处理、传统 CV | ⭐ 80k+ |
| **Ultralytics (YOLOv8)** | 实时目标检测 | 端侧部署、实时应用 | ⭐ 35k+ |
| **MMDetection** | 模型丰富 | 检测任务研究 | ⭐ 29k+ |
...
1. **基础：** OpenCV → 图像处理基础
2. **检测：** YOLOv8 → 实时目标检测
3. **分割：** Detectron2 → 实例分割
4. **生成：** Stable Diffusion → 图像生成
